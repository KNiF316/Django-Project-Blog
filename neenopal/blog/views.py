...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            messages.success(request, 'Congratulations!! You have successfully registered to Blog made easy')
            user = form.save()
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                pst = Post(title=title, createdDate=createdDate, author=author, description=description, isPublished=isPublished)
                pst.save()
                form = PostForm()
        else:
            form = PostForm()
